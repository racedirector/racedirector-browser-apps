        ...
        updateCarNumber = ->
            ...
            element.css
                background: "rgba(#{carClassColor >> 16},\
                    #{carClassColor >> 8 & 0xff},\
                    #{carClassColor & 0xff},\
                    #{if scope.$parent.standings then .75 else 1})"
...
app.controller 'TwitchCtrl', ($scope, $element, $interval, $timeout, $sce, $http, config) ->
    ...
    getChannelID = (callback) ->
        $http.get $sce.trustAsResourceUrl("#{baseUri}/users"),
            params:
                login: channel
            headers:
        ...
        .then (response) ->
            if response.error?
                console.log response.status, response.error, response.message
                setTimeout ->
                    getChannelID callback
                , 5000
                return
    ...
    updateTwitchViewers = ->
        $http.get $sce.trustAsResourceUrl("#{baseUri}/streams/#{channelId}"),
            headers:
                accept: 'application/vnd.twitchtv.v5+json'
                'client-id': clientID
        .then (response) ->
    ...
    getFollowers = (limit, offset, success, error) ->
        $http.get $sce.trustAsResourceUrl("#{baseUri}/channels/#{channelId}/follows"),
            params:
                direction: 'DESC'
                limit: limit
                offset: offset
